...
{
  uint32_t lower = LPC_TIM3->PC;
  uint32_t upper = LPC_TIM3->TC;
  if(lower < LPC_TIM3->PC)
  {
    lower = LPC_TIM3->PC;
    upper = LPC_TIM3->TC;
  }
  return (((us_timestamp_t)upper) << 32) + lower;
}
...
static void sleep_tracker_print_stats(void)
{
  ...
  else
  {
    ...
    for(int i = 0; i < STATISTIC_COUNT; ++i)
    {
      ...
      mbed_error_printf(" [%s x %u]", sleep_stats[i].identifier, sleep_stats[i].count);
    }
  }
  mbed_error_printf("\r\n");
}
